require('dotenv').config();

// v1 - Legacy
const authRoutes = require('./routes/auth.routes');
const merchantRoutes = require('./routes/merchant.routes');
const walletRoutes = require('./routes/wallet.routes');
const cashierRoutes = require('./routes/cashier.routes');
const regionRoutes = require('./routes/region.routes');
const transactionRoutes = require('./routes/transaction.routes');
const refundRoutes = require('./routes/refund.routes');
const agentRoutes = require('./routes/agent.routes');
const enterpriseRoutes = require('./routes/enterprise.routes');
const adminRoutes = require('./routes/admin/admin.routes');
const adminAnalyticsRoutes = require('./routes/admin/admin.analytics.routes'); 
const adminLedgerRoutes = require('./routes/admin/admin.ledger.routes');

// Admin Agent Routes (NOVO)
const adminAgentRoutes = require('./routes/admin/agent.routes');

// Payment Gateway (M-Pesa, e-Mola, etc)
const paymentRoutes = require('./routes/payment.routes');

// Transferências seguras
const secureTransferRoutes = require('./routes/secure-transfer.routes');

// Transaction Domain
const transactionDomainRoutes = require('./routes/transaction.domain.routes');

// Escrow
const escrowRoutes = require('./routes/escrow.routes');

// Confirmation
const confirmationRoutes = require('./routes/confirmation.routes');

// v2 - Modular
const authModuleRoutes = require('./modules/auth/auth.routes');
const identityModuleRoutes = require('./modules/identity/identity.routes');
const uiModuleRoutes = require('./modules/ui/ui.routes');
const accessModuleRoutes = require('./modules/access/access.routes');
const bootstrapModuleRoutes = require('./modules/bootstrap/bootstrap.routes');
const policyModuleRoutes = require('./modules/policy/policy.routes');
const policyAdminRoutes = require('./modules/policy/admin/policy.admin.routes');

// SSE (Server-Sent Events)
const websocketRoutes = require('./websocket/websocket.routes');

// Public Routes
const publicRoutes = require('./routes/public.routes');

// 🧪 DEV ROUTES (apenas em desenvolvimento)
const devRoutes = require('./routes/dev/dev.routes');

// 🩺 HEALTH CHECK (NOVO)
const healthRoutes = require('./health/health.controller');

const cors = require('@fastify/cors');
const rateLimit = require('@fastify/rate-limit');

console.log('🔥 APP.JS CARREGADO!');

module.exports = async function (fastify) {
  console.log('📌 A REGISTAR PLUGINS...');
  
  const { handleError } = require('./utils/errors');

  fastify.setErrorHandler((error, request, reply) => {
    handleError(error, reply);
  });

  fastify.register(cors, {
    origin: true,
    credentials: true
  });

  fastify.register(rateLimit, {
    max: 100,
    timeWindow: '1 minute',
    allowList: [],
    errorResponseBuilder: () => ({
      success: false,
      error: {
        code: 'RATE_LIMIT',
        message: 'Too many requests',
        statusCode: 429
      }
    })
  });

  console.log('📌 A REGISTAR ROTAS...');

  // =========================
  // ROTAS v1 (LEGACY) - CONTINUAM A FUNCIONAR
  // =========================
  await fastify.register(authRoutes, { prefix: '/api/auth' });
  await fastify.register(merchantRoutes, { prefix: '/api/merchant' });
  await fastify.register(walletRoutes, { prefix: '/api/wallet' });
  await fastify.register(cashierRoutes, { prefix: '/api/cashier' });
  const paymentIntentPublicRoutes = require('./routes/payment-intent.public.routes');
  await fastify.register(paymentIntentPublicRoutes, { prefix: '/api/public' });
  await fastify.register(regionRoutes, { prefix: '/api/regions' });
  await fastify.register(transactionRoutes, { prefix: '/api' });
  await fastify.register(refundRoutes, { prefix: '/api' });
  await fastify.register(agentRoutes, { prefix: '/api' });
  await fastify.register(enterpriseRoutes, { prefix: '/api' });
  await fastify.register(adminRoutes, { prefix: '/api/admin' });

  // =========================
  // ADMIN ANALYTICS (NOVO)
  // =========================
  await fastify.register(adminAnalyticsRoutes, { prefix: '/api' });

  // =========================
  // ADMIN LEDGER (NOVO)
  // =========================
  await fastify.register(adminLedgerRoutes, { prefix: '/api' });

  // =========================
  // ADMIN AGENT ROUTES (NOVO)
  // =========================
  await fastify.register(adminAgentRoutes, { prefix: '/api' });

  // =========================
  // PAYMENT GATEWAY (NOVO - M-Pesa, e-Mola, etc)
  // =========================
  await fastify.register(paymentRoutes, { prefix: '/api' });

  // =========================
  // TRANSFERÊNCIAS SEGURAS
  // =========================
  await fastify.register(secureTransferRoutes, { prefix: '/api' });

  // =========================
  // TRANSACTION DOMAIN
  // =========================
  await fastify.register(transactionDomainRoutes, { prefix: '/api' });

  // =========================
  // ESCROW
  // =========================
  await fastify.register(escrowRoutes, { prefix: '/api' });

  // =========================
  // CONFIRMATION
  // =========================
  await fastify.register(confirmationRoutes, { prefix: '/api' });

  // =========================
  // ROTAS v2 (MODULAR)
  // =========================
  await fastify.register(authModuleRoutes, { prefix: '/api/v2/auth' });
  await fastify.register(identityModuleRoutes, { prefix: '/api/v2/identity' });
  await fastify.register(uiModuleRoutes, { prefix: '/api/v2/ui' });
  await fastify.register(accessModuleRoutes, { prefix: '/api/v2/access' });
  await fastify.register(bootstrapModuleRoutes, { prefix: '/api/v2/bootstrap' });
  await fastify.register(policyModuleRoutes, { prefix: '/api/v2/policy' });
  await fastify.register(policyAdminRoutes, { prefix: '/api/v2/policy' });

  // =========================
  // ROTAS PÚBLICAS
  // =========================
  await fastify.register(publicRoutes, { prefix: '/api/public' });

  // =========================
  // SSE (Server-Sent Events)
  // =========================
  await fastify.register(websocketRoutes);

  // =========================
  // 🧪 DEV ROUTES (apenas em desenvolvimento)
  // =========================
  if (process.env.NODE_ENV !== 'production') {
    await fastify.register(devRoutes, { prefix: '/api/dev' });
    console.log('   🧪 Dev routes loaded (testing only)');
  }

  // =========================
  // 🩺 HEALTH CHECK (NOVO - COMPLETO)
  // =========================
  await fastify.register(healthRoutes, { prefix: '/api' });

  console.log('✅ Routes loaded successfully');
  console.log('   🟢 v1 (legacy):');
  console.log('   - /api/auth/*');
  console.log('   - /api/merchant/*');
  console.log('   - /api/wallet/*');
  console.log('   - /api/cashier/*');
  console.log('   - /api/regions/*');
  console.log('   - /api/transfer');
  console.log('   - /api/refund');
  console.log('   - /api/admin/*');
  console.log('   ');
  console.log('   📊 Admin Analytics:');
  console.log('   - /api/admin/dashboard/executive');
  console.log('   - /api/admin/dashboard/financial');
  console.log('   - /api/admin/dashboard/operational');
  console.log('   - /api/admin/dashboard/risk');
  console.log('   - /api/admin/dashboard/system');
  console.log('   - /api/admin/dashboard/ledger');
  console.log('   ');
  console.log('   📊 Admin Ledger:');
  console.log('   - /api/admin/ledger/status');
  console.log('   - /api/admin/ledger/reconcile');
  console.log('   ');
  console.log('   👥 Admin Agents (NOVO):');
  console.log('   - /api/admin/agents');
  console.log('   - /api/admin/agents/stats');
  console.log('   - /api/admin/agents/:kpId');
  console.log('   - /api/admin/agents/:kpId/status');
  console.log('   ');
  console.log('   💳 Payment Gateway:');
  console.log('   - /api/payment/deposit');
  console.log('   - /api/payment/withdraw');
  console.log('   - /api/payment/status/:transactionId');
  console.log('   - /api/payment/transactions');
  console.log('   - /api/payment/webhook/:provider');
  console.log('   ');
  console.log('   🟣 v2 (modular):');
  console.log('   - /api/v2/auth/*');
  console.log('   - /api/v2/identity/*');
  console.log('   - /api/v2/ui/*');
  console.log('   - /api/v2/access/*');
  console.log('   - /api/v2/bootstrap');
  console.log('   - /api/v2/policy/*');
  console.log('   ');
  console.log('   🔐 Transaction Domain:');
  console.log('   - /api/transaction/*');
  console.log('   ');
  console.log('   🔐 Escrow:');
  console.log('   - /api/escrow/*');
  console.log('   ');
  console.log('   🔐 Confirmation:');
  console.log('   - /api/confirmation/*');
  console.log('   ');
  console.log('   🌐 Public Routes:');
  console.log('   - /api/public/bootstrap (público)');
  console.log('   - /api/public/status');
  console.log('   - /api/public/health');
  console.log('   ');
  console.log('   🩺 Health Check (NOVO):');
  console.log('   - /api/health');
  console.log('   - /api/health/ping');
  console.log('   - /api/health/detailed');
  console.log('   ');
  console.log('   🔌 WebSocket:');
  console.log('   - ws://localhost:3000/ws');

  fastify.setNotFoundHandler((request, reply) => {
    reply.status(404).send({
      success: false,
      error: {
        code: 'NOT_FOUND',
        message: 'Route not found',
        statusCode: 404
      }
    });
  });

  console.log('✅ APP.JS CONFIGURADO COM SUCESSO!');
  return fastify;
};