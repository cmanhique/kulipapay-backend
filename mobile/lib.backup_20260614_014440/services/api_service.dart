...
class ApiService {
  ...
  Future<Map<String, dynamic>> getAdminUsers({int page = 1, String search = ''}) async {
    final headers = await _getHeaders();
    final response = await http.get(
      Uri.parse('${AppConfig.apiUrl}/api/admin/users?page=$page&limit=50&search=$search'),
      headers: headers,
    );
    if (response.statusCode == 200) {
      return json.decode(response.body);
  ...
  Future<void> toggleBlockUser(String kpId) async {
    final headers = await _getHeaders();
    final response = await http.post(
      Uri.parse('${AppConfig.apiUrl}/api/admin/users/$kpId/toggle-block'),
      headers: headers,
    );
    if (response.statusCode != 200) {
